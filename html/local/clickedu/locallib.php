 ...
 * Global library file
 *
 * @author      Adri脿 Monguillot <amonguillot@clickedu.net>
 * @author      Daniel Ure帽a Zafra <durenadev@gmail.com>
 * @copyright   2019 CLICKART TALLER DE COMUNICACI脫, S.L.
...
// Clave de la aplicaci贸n proporcionado por ClickArt
...
function clickedu_login($username, $password) {
    ...
    // Obtenci贸n del token y secret de usuario
    ...
    // Login de usuario y obtenci贸n del id
...
function clickedu_sync_contents(stdClass $course, array $contents, progress_bar $progress) {
    ...
    foreach ($contents as $sectionnum => $sectioncontents) {
        ...
        // Lista de m貌dulos actuales de la secci贸n
        $cms = array();
        ...
        // Creaci贸n / actualizaci贸n de recursos
        foreach ($sectioncontents['modules'] as $idnumber => $modulecontents) {
            // Avanza hasta el siguiente m贸dulo de Clickedu no procesado
            while ($cm and
    ...
    // Eliminaci贸n de recursos que ya no existen en Clickedu
...
function clickedu_sync_courses(array $courses, progress_bar $progress) {
    ...
    if (!$category or !$category->id) {
        ...
        /* 
            maguilera: Mejora para a馻dir y detectar si existe un curso escolar y crearlo como categoria
        */
...
function clickedu_create_users(array $users, progress_bar $progress) {
    ...
    foreach ($users as $user) {
        /* maguilera: Hago una comprobaci髇 porque al pasar el codigo del colegio por WS el primer indice ya no es un object(user)*/
