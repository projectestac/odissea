...
function AuthenticateUserContent($data, $usr_creden = false, $showurl = true) {
    ...
    if (!($publisher = $DB->get_record('rcommon_publisher', array('id' => $book->publisherid)))) {
    ...
    if (!empty($data->unitid) && ($unit = $DB->get_record('rcommon_books_units', array('id' => $data->unitid))) == false) {
        throw new \moodle_exception('nounit', 'block_rcommon');
    }
    ...
    if ($response->AutenticarUsuarioContenidoResult->Codigo <= 0) {
        ...
        throw new \moodle_exception('error_authentication', 'local_rcommon', $response->AutenticarUsuarioContenidoResult->Codigo . '<br>' . $desctext . $msg);
    }
