...
switch ($action) {
    case 'edit':
        ...
        if (!$record = $DB->get_record('rcommon_publisher', array('id' => $publisher))) {
            throw new \moodle_exception('nopublisher', 'local_rcommon', $CFG->wwwroot . '/local/rcommon/publishers.php');
        }
    case 'add';
        if ($fromform = $bform->get_data() and confirm_sesskey()) {
            ...
            if (empty($record->name)) {
                throw new \moodle_exception('savekoemptyvalues', 'local_rcommon', $CFG->wwwroot . '/local/rcommon/publishers.php');
            }
    ...
    default:
        ...
        $sql = 'SELECT p.id, p.name, p.code, count(b.id) as books
                ...
                LEFT JOIN {rcommon_books} b ON b.publisherid = p.id
