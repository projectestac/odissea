<?php

// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * Strings for component 'currencies', language 'gl', branch 'MOODLE_31_STABLE'
 *
 * @package   currencies
 * @copyright 1999 onwards Martin Dougiamas  {@link http://moodle.com}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

$string['AED'] = 'Dirham dos Emiratos Árabes Unidos';
$string['AFN'] = 'Afgani afgán';
$string['ALL'] = 'Lek albanés';
$string['AMD'] = 'Dram da Armenia';
$string['ANG'] = 'Guilder das Antillas Holandesas';
$string['AOA'] = 'Kwanza angolano';
$string['ARS'] = 'Peso arxentino';
$string['AUD'] = 'Dólar australiano';
$string['AWG'] = 'Florín de Aruba';
$string['AZN'] = 'Manat acerbaixano';
$string['BAM'] = 'Marco convertíbel de Bosnia e Herzegovina';
$string['BBD'] = 'Dólar das Barbados';
$string['BDT'] = 'Taka bangladechiano';
$string['BGN'] = 'Lev búlgaro';
$string['BHD'] = 'Dinar de Barein';
$string['BIF'] = 'Franco burundés';
$string['BMD'] = 'Dólar das Bermudas';
$string['BND'] = 'Dólar do Brunei';
$string['BOB'] = 'Boliviano de Bolivia';
$string['BRL'] = 'Real brasileiro';
$string['BSD'] = 'Dólar das Bahamas';
$string['BTN'] = 'Ngultrum butanés';
$string['BWP'] = 'Pula botswaniano';
$string['BYR'] = 'Rublo Bielorruso';
$string['BZD'] = 'Dólar do Belize';
$string['CAD'] = 'Dólar do Canadá';
$string['CDF'] = 'Franco congolés';
$string['CHF'] = 'Franco suízo';
$string['CLF'] = 'Unidades de Fomento Chilenas';
$string['CLP'] = 'Peso chileno';
$string['CNY'] = 'Yuan renminbi chinés';
$string['COP'] = 'Peso colombiano';
$string['CRC'] = 'Colón da Costa Rica';
$string['CUC'] = 'Peso cubano convertíbel';
$string['CUP'] = 'Peso cubano';
$string['CVE'] = 'Escudo caboverdiano';
$string['CZK'] = 'Coroa checa';
$string['DJF'] = 'Franco do Xibutí';
$string['DKK'] = 'Coroa dinamarquesa';
$string['DOP'] = 'Peso dominicano';
$string['DZD'] = 'Dinar arxelino';
$string['EGP'] = 'Libra exipcia';
$string['ERN'] = 'Nakfa eritreo';
$string['ETB'] = 'Birr etíope';
$string['EUR'] = 'Euro';
$string['FJD'] = 'Dólar de Fidxi';
$string['FKP'] = 'Libra das Illas Falkland';
$string['GBP'] = 'Libra Esterlina';
$string['GEL'] = 'Lari xeorxiano';
$string['GHS'] = 'Cedi ganés';
$string['GIP'] = 'Libra de Xibraltar';
$string['GMD'] = 'Dalasi gambiano';
$string['GNF'] = 'Franco guineano';
$string['GTQ'] = 'Quetzal guatemalteco';
$string['GYD'] = 'Dólar da Güiana';
$string['HKD'] = 'Dólar do Hong kong';
$string['HNL'] = 'Lempira hondureña';
$string['HRK'] = 'Kuna croata';
$string['HTG'] = 'Gurde haitiano';
$string['HUF'] = 'Forint húngaro';
$string['IDR'] = 'Tupía indonesia';
$string['ILS'] = 'Sheqel israelí';
$string['INR'] = 'Rupia da Índia';
$string['IQD'] = 'Dinar iraquí';
$string['IRR'] = 'Rial iraniano';
$string['ISK'] = 'Coroa islandesa';
$string['JMD'] = 'Dólar xamaicano';
$string['JOD'] = 'Dinar xordano';
$string['JPY'] = 'Ien xaponés';
$string['KES'] = 'Xelín queniano';
$string['KGS'] = 'Som quirguiz';
$string['KHR'] = 'Riel camboxano';
$string['KMF'] = 'Franco das Comores';
$string['KPW'] = 'Won norcoreano';
$string['KRW'] = 'Won surcoreano';
$string['KWD'] = 'Dinar kuwaití';
$string['KYD'] = 'Dólar das Illas Caimán';
$string['KZT'] = 'Tenge cazaquistanés';
$string['LAK'] = 'Kip laosiano';
$string['LBP'] = 'Libra libanesa';
$string['LKR'] = 'Rupia de Sri Lanka';
$string['LRD'] = 'Dólar liberiano';
$string['LSL'] = 'Loti lesotiano';
$string['LTL'] = 'Litas lituano';
$string['LVL'] = 'Lats letón';
$string['LYD'] = 'Dinar libio';
$string['MAD'] = 'Dirham marroquío';
$string['MDL'] = 'Leu moldavo';
$string['MGA'] = 'Franco malgaxe';
$string['MKD'] = 'Denar macedonio';
$string['MMK'] = 'Kyat de Myanmar (birmano)';
$string['MNT'] = 'Tugrik mongol';
$string['MOP'] = 'Pataca de Macao';
$string['MRO'] = 'Ouguiya mauritano';
$string['MUR'] = 'Rupia mauricia';
$string['MVR'] = 'Rufiyaa maldiva';
$string['MWK'] = 'Kwacha malawiano';
$string['MXN'] = 'Peso mexicano';
$string['MYR'] = 'Ringgit malaio';
$string['MZN'] = 'Metical mozambicano';
$string['NAD'] = 'Dólar namibio';
$string['NGN'] = 'Naira nixeriana';
$string['NIO'] = 'Córdoba oro nicaraguano';
$string['NOK'] = 'Coroa norueguesa';
$string['NPR'] = 'Tupía nepalesa';
$string['NZD'] = 'Dólar da Novacelandés';
$string['OMR'] = 'Rial omaní';
$string['PAB'] = 'Balboa panameño';
$string['PEN'] = 'Novo sol peruano';
$string['PGK'] = 'Kina de Papúa Nova Guinea';
$string['PHP'] = 'Peso filipino';
$string['PKR'] = 'Tupía paquistaní';
$string['PLN'] = 'Zloti polaco';
$string['PYG'] = 'Guaraní paraguaio';
$string['QAR'] = 'Rial qatarí';
$string['RON'] = 'Leu romeno';
$string['RSD'] = 'Dinar serbio';
$string['RUB'] = 'Rublo ruso';
$string['RWF'] = 'Franco ruandés';
$string['SAR'] = 'Riyal saudí';
$string['SBD'] = 'Dólar das Illas Salomón';
$string['SCR'] = 'Tupía das Seicheles';
$string['SDG'] = 'Libra sudanesa';
$string['SEK'] = 'Coroa sueca';
$string['SGD'] = 'Dólar de Singapur';
$string['SHP'] = 'Libra de Santa Helena';
$string['SLL'] = 'Leone de Serra Leoa';
$string['SOS'] = 'Xelín somaliano';
$string['SRD'] = 'Dólar do Surinam';
$string['SSP'] = 'Libra sursudanesa';
$string['STD'] = 'Dobra de San Tomé e Príncipe';
$string['SVC'] = 'Colón salvadoreño';
$string['SYP'] = 'Libra siria';
$string['SZL'] = 'Lilangeni suazilandés';
$string['THB'] = 'Baht tailandés';
$string['TJS'] = 'Somoni taxiquistanés';
$string['TMT'] = 'Manat do Turquemenistán';
$string['TND'] = 'Dinar tunisiano';
$string['TOP'] = 'Pa`anga tongano';
$string['TRY'] = 'Lira turca';
$string['TTD'] = 'Dólar de Trindade e Tobago';
$string['TWD'] = 'Dólar taiwanés';
$string['TZS'] = 'Xelín tanzano';
$string['UAH'] = 'Hryvnia ucranína';
$string['UGX'] = 'Xelín ugandés';
$string['USD'] = 'Dólar dos EUA';
$string['UYU'] = 'Peso uruguaio';
$string['UZS'] = 'Sum usbequistanés';
$string['VEF'] = 'Bolívar Fuerte venezolano';
$string['VND'] = 'Dong vietnamita';
$string['VUV'] = 'Vatu vanuatense';
$string['WST'] = 'Tala samoana';
$string['XAF'] = 'Franco CFA BEAC';
$string['XCD'] = 'Dólar do Caribe Oriental';
$string['XOF'] = 'Franco CFA BCEAO';
$string['XPF'] = 'Franco CFP';
$string['YER'] = 'Riyal iemení';
$string['ZAR'] = 'Rand sudafricano';
$string['ZMK'] = 'Kwacha de Zambia';
$string['ZWL'] = 'Dólar do Cimbabue';
