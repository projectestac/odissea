...
$string['alreadyexporting'] = 'このセッションには、すでにアクティブなポートフォリオエクスポートがあります。続ける前にあなたはこのエクスポートを完了するか、キャンセルする必要があります。続けてもよろしいですか? (Noを選択するとキャンセルされます)';
...
$string['commonsettingsdesc'] = '<p>「中程度」または「高程度」の転送時間を必要だと見なされる状態に対して、ユーザが転送完了を待つことができるかどうか設定します。</p><p>「中程度」の閾値に達するまで、ユーザへの問いかけなしに処理が実行されます。また、「中程度」および「高程度」の転送ではユーザに対してオプションが提示されますが、時間を要することが警告されます。</p><p>さらに、いくつかのポートフォリオプラグインでは、このオプションを完全に無視してすべての転送を強制的にキューに入れる場合があります。</p>';
...
$string['exportexpireddesc'] = 'あなたはいくつかの情報を繰り返しエクスポートしようとしているか、空のエクスポートを開始しようとしています。適切にエクスポートするには、オリジナルのロケーションに戻って再度エクスポートを開始する必要があります。エクスポートの後、あなたがブラウザの戻るボタンを使用したか無効なURLをブックマークした場合、この現象が時々発生します。';
...
{$a->htmllist}
<p><a href="{$a->fixurl}">ポートフォリオ設定ページ</a>にアクセスして、この問題を可能な限り早く修正してください。</p>';
$string['insanebodysmall'] = 'こんにちは! あなたはこのメッセージを {$a->sitename} の管理者として受信しています。設定が正しくないため、いくつかのポートフォリオプラグインが自動的に無効にされました。これは現在のところ、ユーザがこれらのポートフォリオにコンテンツをエクスポートできないことを意味します。{$a->fixurl} にアクセスして、この問題を可能な限り早く修正してください。';
