...
<p>指定した名称のカテゴリがない場合、隠されたカテゴリが作成されます。</p>';
$string['createnewcourses'] = 'Moodleに登録されていない場合、新しい (非表示) コースを作成する';
$string['createnewcourses_desc'] = 'この設定を有効にした場合、IMSエンタープライズ・ユーザ登録プラグインではIMSデータで見つかったMoodleデータベースには登録されていないコースを新たに作成することができます。すべての新しく作成されたコースは非表示とされます。';
...
ユーザは最初に「idnumber」で、次にMoodleユーザ名で検索されます。パスワードはIMSエンタープライズプラグインによってインポートされません。Moodle認証プラグインでユーザ認証することをお勧めします。';
$string['cronfrequency'] = '処理の頻度';
$string['deleteusers'] = 'IMSデータに指定されている場合、ユーザアカウントを削除する';
$string['deleteusers_desc'] = 'この設定を有効にした場合、IMSエンタープライズ・ユーザ登録データではユーザアカウントの削除を指定  (「recstatus」フラグに、ユーザアカウントの削除を意味する 3 を設定) することができます。Moodleの標準どおり、実際にはユーザレコードはMoodleデータベースから削除されず、アカウントに削除フラグがセットされます。';
