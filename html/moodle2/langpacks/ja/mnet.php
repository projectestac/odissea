...
$string['error7020'] = '通常、このエラーはリモートサイトが正しくないwwwrootレコードを作成した場合に発生します。例えば、http://www.yoursite.comの代わりにhttp://yoursite.comを使用した場合です。あなたのwwwroot (config.phpで設定) をリモートサイトの管理者に伝えて、あなたのサイトに関するレコードの変更を依頼してください。';
...
$string['expireyourkeyexplain'] = 'Moodleは (デフォルトで) 28日ごとに自動的に公開鍵をローテートしますが、あなたはこの公開鍵をいつでも「手動」で失効させることができます。これはあなたがこのキーを信用できなくなった場合のみ有用です。公開鍵の交換は即座に自動実行されます。<br />この公開鍵を削除することで、あなたが手動でそれぞれのMoodle管理者に連絡して新しいキーを提供するまで、他のシステムと通信できなくなります。';
...
$string['mnetidprovidermsg'] = 'あなたの {$a} プロバイダにログインすることができます。';
...
$string['nosuchfunction'] = 'RPCの関数を設定できないか、禁止されています。';
$string['nosuchmodule'] = '誤って実行されたため、関数を探すことができません。mod/modulename/lib/functionnameの形式を使用してください。';
...
$string['ssoacldescr'] = 'このページはリモートMoodleネットワークホストのユーザによるアクセスを許可/拒否するため使用してください。この機能はあなたがリモートユーザにSSOサービスを提供するときに動作します。あなたの「ローカル」ユーザが他のMoodleネットワークホストを散策できるような権限をコントロールするにはロールシステムにてユーザに「mnetlogintoremote」ケイパビリティを付与してください。';
...
$string['trustedhostsexplain'] = '<p>信頼できるホストメカニズムでは、特定のマシンにXML-RPC経由であらゆるMoodle APIの一部を実行することを許可します。Moodleの挙動をコントロールするスクリプトの実行が可能になり、このオプションを有効にすることで非常に危険になります。分からない場合、無効のままにしてください。</p>
