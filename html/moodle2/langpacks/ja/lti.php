...
/**
 * Strings for component 'lti', language 'ja', branch 'MOODLE_28_STABLE'
 *
 * @package   lti
...
$string['capabilities_help'] = 'あなたがツールプロバイダに提供するケイパビリティを選択してください。1つ以上のケイパビリティを選択してください。';
...
$string['custom_help'] = 'カスタムパラメータはツールプロバイダで使用される設定です。例えば、プロバイダから特定のリソースを表示するため、カスタムパラメータを使用することができます。それぞれのパラメータは独立した行に「name=value」のような形で入力する必要があります。例えば、次のようになります: 「chapter=3」
...
$string['delete'] = '削除';
$string['delete_confirmation'] = '本当にこの外部ツール設定を削除してもよろしいですか?';
...
$string['external_tool_type_help'] = 'ツール設定の主目的はMoodleおよびツールプロバイダ間の安全な通信チャネルを構築することにあります。
...
* **特定のツールタイプ** - 特定のツールタイプを選択することであなたは外部ツールプロバイダとの通信に関してMoodleにツール設定内容の使用を強制することができます。起動URLがツールプロバイダに属していない場合、警告が表示されます。一部のケースでは特定ツールの提供時に起動URLを入力する必要はありません (ツールプロバイダ内で特定のリソースを起動しな場合)。
...
外部ツールタイプのドロップダウンメニューでは3つのアイコンを選択することができます:

* **追加** - コースレベルの設定を作成します。コース内すべての外部ツールインスタンスではツール設定を使用することができます。

* **編集** - コースレベルのツールタイプをドロップダウンメニューから選択してアイコンをクリックしてください。ツール設定詳細を編集することができます。
...
$string['fixexistingconf'] = '設定の正しくないインスタンスに対して既存の設定を使用する';
$string['fixnew'] = '新しい設定';
$string['fixnewconf'] = '正しくない設定のインスタンスに対して新しい設定を定義する';
...
$string['force_ssl_help'] = 'このオプションを有効にすることにより、このツールプロバイダのツール起動すべてにSSLが使用されます。
...
$string['icon_url_help'] = 'アイコンURLではこの活動がコース一覧に表示されるときに表示されるアイコンを変更します。デフォルトのLTIアイコンを使用する代わりに活動のタイプで使用されるアイコンを指定することができます。';
$string['id'] = 'ID';
...
$string['launchinpopup_help'] = '起動コンテナはコースからツールが起動される場合の表示に影響します。いくつかの起動コンテナではツールに場所を提供して、他の起動コンテナではMoodle環境との統合を実現します。
...
$string['lti_tool_request_added'] = 'ツール設定のリクエストが正常に送信されました。あなたはツール設定を完了するために管理者に連絡する必要があります。';
...
$string['manage_tools'] = '外部ツールタイプを管理する';
...
$string['parameter_help'] = 'ツールパラメータは認められたツールプロクシ内でツールプロバイダによりパスされるために要求される設定です。';
...
$string['password_help'] = '設定処理の一環として提供されるため、事前に設定されたツールではここで共通秘密鍵を入力する必要はありません。
...
$string['register_warning'] = '登録ページを開くのにしばらく時間がかかります。表示されない場合、あなたが設定で正しいURLを入力したか確認してください。';
...
$string['registrationurl_help'] = '登録リクエストが送信されるロケーションとして、登録URLはツールプロバイダから利用することができます。';
$string['reject'] = '拒否';
$string['rejected'] = '拒否';
$string['resource'] = 'リソース';
$string['resourcekey'] = 'コンシューマ鍵';
$string['resourcekey_admin'] = 'コンシューマ鍵';
$string['resourcekey_admin_help'] = 'コンシューマ鍵はツールへの認証アクセスに使用されるユーザ名だと考えることができます。ユーザによるMoodleサイトからのツール起動を一意的に識別するため、ツールプロバイダが使用することができます。

コンシューマ鍵はツールプロバイダから提供される必要があります。コンシューマ鍵の取得方法はツールプロバイダ間で異なります。自動処理である場合も、またツールプロバイダに連絡が必要な場合もあります。

Moodleからの安全な通信を必要とせず、評定レポートのような追加的なサービスを提供しない場合、コンシューマ鍵は必要ではありません。';
$string['resourcekey_help'] = '設定処理の一環として提供されるため、事前に設定されたツールではここでコンシューマ鍵を入力する必要はありません。

未設定のツールプロバイダへのリンクを作成する場合、このフィールドには設定値を入力する必要があります。このコース内でツールプロバイダが1回以上使用される場合、コースツール設定を追加することは良い考え方です。

コンシューマ鍵はツールへの認証アクセスに使用されるユーザ名だと考えることができます。ユーザによるMoodleサイトからのツール起動を一意的に識別するため、ツールプロバイダが使用することができます。
...
$string['services_help'] = 'あなたがツールプロバイダに提供したいサービスを選択してください。1つ以上のサービスを選択することができます。';
$string['setupoptions'] = 'セットアップオプション';
$string['share_email'] = 'ランチャEメールをツールと共有する';
$string['share_email_admin'] = 'ランチャEメールをツールと共有する';
...
ツール内に有益な情報を表示するため、ツールプロバイダは起動者の名前を必要とします。

この設定はツール設定内でオーバーライドできることに留意してください。';
$string['share_roster'] = 'このコースの参加者一覧へのツールのアクセスを許可する';
$string['share_roster_admin'] = 'ツールがコース一覧にアクセスできる';
$string['share_roster_admin_help'] = 'このツールタイプが起動された場合、コース内の登録済みユーザ一覧にツールがアクセスできるかどうか指定してください。';
...
このオプションが選択されるべき唯一のケースはツール設定がシングルサインオンを意図している場合のみです。
...
$string['toolproxy_help'] = '外部ツール登録はMoodleサイト管理者がLTI 2.0をサポートしているツールプロバイダから取得したツールプロクシから外部ツールを設定できるようにします。ツールプロバイダから提供されたURLすべてにおいて、処理を開始できる必要があります。ツールプロバイダに提供するケイパビリティおよびサービスは新しい登録の設定時に選択されます。

このページに一覧表示されているツール登録は4つのカテゴリに分けられます:

* **設定** - これらのツール登録はセットアップされていますが、登録処理は開始していません。
* **保留** - これらのツール登録の登録処理は開始していますが、完了していません。 「設定」カテゴリに戻すには、設定を開いて保存してください。
* **承認** - これらのツール登録は承認されています。ツールプロクシで指定されたリソースは初期ステータス「保留」の状態で外部ツールタイプページに表示されます。
* **拒否** - これらのツール登録は登録処理中に拒否されています。登録処理を再開するため「設定」カテゴリに戻すには、設定を開いて保存してください。';
...
$string['typename_help'] = 'ツール名はMoodle内のツールプロバイダを識別するために使用されます。ここで入力された名称はコース内で外部ツールを追加するときに教師に表示されます。';
$string['types'] = 'タイプ';
$string['update'] = '更新';
