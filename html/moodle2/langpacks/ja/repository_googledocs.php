...
/**
 * Strings for component 'repository_googledocs', language 'ja', branch 'MOODLE_34_STABLE'
 *
...
$string['fileoptions'] = 'ここではタイプおよび戻されるファイルのデフォルトを設定します。外部にリンクされているすべてのファイルが更新されるため、オーナーがMoodleシステムアカウントになる点に留意してください。';
...
$string['issuer_help'] = 'Google Drive APIと通信するために設定するOAuth 2サービスを選択してください。まだサービスが存在していない場合、あなたは作成する必要があります。';
$string['oauth2serviceslink'] = '<a href="{$a}" title="OAuthサービス設定へのリンク">OAuth 2サービス設定</a>';
$string['oauthinfo'] = '<p>このプラグインを使用するにはドキュメンテーション「<a href="{$a->docsurl}">Google OAuth 2.0セットアップ</a>」に記載されているようにあなたのサイトをGoogleに登録する必要があります。</p><p>登録処理の一環として、あなたは次のURLを「公式リダイレクトURL」として入力する必要があります:</p><p>{$a->callbackurl}</p><p>登録後、他の特定のGoogle DriveおよびPicasaプラグインの設定に使用することのできるクライアントIDおよび秘密鍵が提供されます。</p>';
$string['owner'] = 'オーナー: {$a}';
$string['pluginname'] = 'Google Drive';
$string['presentationformat'] = 'デフォルトのプレゼンテーションインポートフォーマット';
$string['privacy:metadata:repository_googledocs'] = 'Google Driveリポジトリプラグインはどのような個人データも保存しません。しかし、ユーザデータをMoodleからリモートシステムに転送します。';
