...
/**
 * Strings for component 'portfolio_mahara', language 'ja', branch 'MOODLE_32_STABLE'
 *
...
$string['enableleap2a'] = 'Leap2Aポートフォリオサポートを有効にする (要Mahara 1.3以上)';
...
$string['err_networkingoff_help'] = 'MNetは完全に無効にされています。このプラグインを設定する前に有効にしてください。問題が修正されるまでこのプラグインに関するインスタンスはすべて非表示にされます - 再度、手動でインスタンスを表示する必要があります。インスタンスが表示されるまで使用することはできません。';
...
$string['err_nomnethosts_help'] = 'このプラグインではMoodleネットワーキングピアのSSO IDP (アイデンティティプロバイダ) の公開、ポートフォリオおよびSSO SP (サービスプロバイダ) の登録に依存します。また、同様にMNet認証プラグインにも依存します。問題が修正されるまでこのプラグインに関するインスタンスはすべて非表示にされます - 再度、手動でインスタンスを表示する必要があります。インスタンスが表示されるまで使用することはできません。';
...
$string['mnet_wronghost'] = 'この転送のトークンにリモートホストが合致しません。';
