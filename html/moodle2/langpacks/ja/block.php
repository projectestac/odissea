...
/**
 * Strings for component 'block', language 'ja', branch 'MOODLE_28_STABLE'
 *
 * @package   block
...
$string['contexts_help'] = 'コンテクストはオリジナルブロックのロケーション内でこのブロックを表示することのできる、さらに特定のページのタイプです。オリジナルブロックのロケーションおよび現在のロケーションに応じて、あなたには異なるオプションがあります。例えば、コースにブロックを追加して (すべてのサブページに表示されるようにした後) フォーラム内に移動します。再度ブロック設定を編集してフォーラムページ内でのみ表示されるよう制限することで、あなたはコース内のフォーラムページのみでブロックを表示するよう制限することができます。';
$string['createdat'] = 'オリジナルブロックロケーション';
$string['createdat_help'] = 'ブロックが作成されるオリジナルロケーションです。ブロック設定によりオリジナルロケーション内の他のロケーション (コンテクスト) にブロックを表示することができます。例えば、コースページに作成されたブロックはそのコースの活動内に表示させることもできます。フロントページに作成されたブロックはサイト全体を通して表示させることができます。';
...
$string['defaultweight_help'] = 'デフォルトの加重では最上部または最下部等、あなたがブロックを表示したい場所を大まかに選択することができます。最終的なロケーションはその場所のブロックすべてから計算されます (例えば、ブロック1つのみの場合、実質的にはトップに表示できます)。必要であれば、特定のページ上でこの値をオーバーライドすることができます。';
