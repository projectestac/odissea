...
$string['noprofileservice'] = 'Сервис профиля не найден';
...
$string['registrationname'] = 'Имя поставщика приложения';
...
$string['toolproxy_help'] = 'Регистрация «Внешнего инструмента» позволяет администраторам сайта Moodle настроить внешние инструменты с доверенностью, полученной от поставщика инструмента, поддерживающего LTI 2,0.  URL-адрес регистрации предоставляется поставщиком инструмента и является всем необходимым, что потребуется, чтобы начать процесс. Возможности и службы, предлагаемые поставщиком инструмента, выбираются при настройке новой регистрации.

Регистрации инструментов, перечисленные на этой странице разделены на четыре категории:
* **Настроено** - Этот регистрируемый инструмент был создан, но процесс регистрации еще не начат.
* **В ожидании** - Процесс регистрации для этого инструмента была начат, но не завершен. Откройте и сохраните настройки, чтобы переместить его обратно в категорию «Настроено».
* **Принято** - Регистрация этого инструмента была утверждена; ресурсы, указанные в доверенности инструмента, появятся на странице «Типы внешних инструментов» с начальным статусом «В ожидании».
* **Отклонено** - Регистрация этого инструмента была отклонена. Откройте и сохраните настройки, чтобы переместить его обратно в категорию «Настроено», чтобы перезапустить заново процесс регистрации.';
