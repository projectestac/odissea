...
/**
 * Strings for component 'cachestore_memcached', language 'ru', branch 'MOODLE_28_STABLE'
 *
...
$string['servers_help'] = 'Перечень серверов, которые  будут использованы как устройства кэширования в памяти. Серверы должны быть перечислены по одному на строке и состоять из адреса сервера, порта и веса. Если порт не представлен, то используется порт по умолчаниют (11211) .

Например:
<pre>
server.url.com
IP-адрес:порт
имя_сервера:порт:вес
</pre>

Если параметр *Включить кластер серверов* включен ниже, то здесь должен быть упомянут только один сервер. Как правило, это имя локального компьютера, например: 127.0.0.1 или localhost.';
