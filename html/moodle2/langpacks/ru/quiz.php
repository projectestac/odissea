...
/**
 * Strings for component 'quiz', language 'ru', branch 'MOODLE_34_STABLE'
 *
 * @package   quiz
...
$string['canredoquestions_desc'] = 'Если параметр включен, то после ответа студентов на конкретный вопрос они увидят кнопку «Повторно ответить на вопрос». Это позволяет им выбрать другой вариант ответа на этот же вопрос, не завершая весь тест и не начиная новый. Такая возможность в основном полезна для тренировочных тестов.

Эта настройка влияет только на те вопросы (например, не на вопросы Эссе) и поведение (например, немедленный отзыв или интерактивный с несколькими попытками), где возможно завершение студентом вопроса до того, как попытка будет отправлена.';
...
$string['indicator:cognitivedepth_help'] = 'Этот показатель основан на  глубине познания, достигнутой студентом при работе с активным элементом Тест.';
...
Это усложнит студентам обмен ответами между собой, но также усложнит для них обсуждение конкретного вопроса с Вами.';
