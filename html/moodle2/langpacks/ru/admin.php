...
/**
 * Strings for component 'admin', language 'ru', branch 'MOODLE_31_STABLE'
 *
 * @package   admin
...
$string['allowguestmymoodle'] = 'Разрешить гостевой доступ к странице «Моя домашняя страница»';
...
$string['availablesearchareas'] = 'Доступные направления для поиска';
...
$string['configallowguestmymoodle'] = 'Открывает гостевой доступ к странице «Моя домашняя страница». Если отключено, то гость перенаправляется на главную страницу сайта.';
...
$string['configcustommenuitems'] = 'Здесь Вы можете настроить пользовательское меню, отображаемое в темах. Каждая строка меню состоит из текста, URL ссылки (не обязательно) и всплывающей подсказки (не обязательно) и кода языка или списка отделенных запятыми кодов (не обязательно, для отображения пользователям строки только на указанном языке). Всё вышеперечисленное разделяется вертикальной чертой. Уровни структуры можно задать с помощью дефисов, в качестве разделителей можно использовать один или несколько символов #. Например:
<pre>
...
--German Moodle Docs|http://docs.moodle.org/de|Документация на немецком|de
#####
Moodle.com|http://moodle.com/
</pre>';
$string['configcustomusermenuitems'] = 'Вы можете настроить содержимое пользовательского меню (за исключением ссылки «Выход», которая добавляется автоматически). Каждая строка разделяется символами | и состоит из: 1) строки в форме «названиеязыка, названиекомпонента» или обычного текста, 2) адреса URL и 3) иконки в виде картинки или в виде URL. Разделители могут использоваться в нужных местах путем добавления строки из одного или нескольких символов #.';
...
$string['customusermenuitems'] = 'Элементы меню пользователя';
...
$string['modeditingmenu'] = 'Меню редактирования элемента курса';
$string['modeditingmenu_desc'] = 'Если параметр включен, то в режиме редактирования значки редактирования элементов курса будут показаны в  выпадающем меню. Это уменьшает содержимое на экране, скрывая значки, пока они не понадобятся.';
...
$string['mypage'] = 'Настройки по умолчанию для страницы «Моя домашняя страница»';
...
$string['navexpandmycourses'] = 'Отображать развернутым блок «Мои курсы» на странице «Моя домашняя страница»';
$string['navexpandmycourses_desc'] = 'При включенном параметре блок «Мои курсы» изначально отображается развернутым в навигационном блоке на странице «Моя домашняя страница».';
...
$string['pathtogs_help'] = 'В Linux это можно оставить как \'/usr/bin/gs\'. В Windows это будет нечто вроде \'c:gsbingswin32c.exe\' (убедитесь, что в пути нет пробелов  - при необходимости скопируйте файлы \'gswin32c.exe\' и \'gsdll32.dll\' в новую папку без пробелов в пути)';
...
$string['pathtounoconv_help'] = 'Путь к конвертеру документов unoconv. Это исполняемый файл, который может преобразовывать документ между форматами,  поддерживаемыми LibreOffice. Это необязательно, но если путь указан, то Moodle использует его для автоматического преобразования между форматами документа. Это используется для поддержки более широкого диапазона входных файлов с возможностью подачи примечаний PDF.';
...
$string['registration_help'] = 'Рекомендуется зарегистрировать свой сайт, чтобы получать  предупреждения о безопасности и иметь доступ к Moodle.net - нашей платформе распространения курсов.';
...
$string['taskcompletionregular'] = 'Расчет стандартных данных, связанных с заверением курса';
