...
function com_wiris_quizzes_impl_HTMLTools_1(&$»this, &$c, &$children, &$elem, &$index, &$it, &$mis, &$mitexts, &$text, &$words) {
	if($index < $children->length) {
		return $children[$index];
	}
}
function com_wiris_quizzes_impl_HTMLTools_2(&$»this, &$c, &$children, &$elem, &$first, &$index, &$it, &$words) {
	if($index < $children->length) {
		return $children[$index];
	}
}
function com_wiris_quizzes_impl_HTMLTools_3(&$»this, &$c, &$children, &$elem, &$first, &$index, &$it, &$mns, &$num, &$words) {
	if($index < $children->length) {
		return $children[$index];
	}
}
function com_wiris_quizzes_impl_HTMLTools_4(&$»this, &$c, &$children, &$elem, &$index, &$it, &$words) {
	if($index < $children->length) {
		return $children[$index];
	}
}
function com_wiris_quizzes_impl_HTMLTools_5(&$»this, &$allowedTags, &$beginformula, &$end, &$formula, &$lasttag, &$omittedcontent, &$pos2, &$spacepos, &$stack, &$start, &$tag, &$text, &$trimmedTag) {
	{
		$s = new haxe_Utf8(null);
		$s->addChar(160);
		return $s->toString();
...
function com_wiris_quizzes_impl_HTMLTools_7(&$»this, &$_g, &$c, &$i, &$j, &$j1, &$m, &$mathml, &$n, &$text, &$tok, &$token, &$tokens) {
	{
		$s = new haxe_Utf8(null);
...
function com_wiris_quizzes_impl_HTMLTools_9(&$»this, &$appendpos, &$character, &$firstchar, &$formula, &$initag, &$length, &$parentpos, &$parenttag, &$parenttagname, &$pos, &$start, &$text) {
	{
		$s = new haxe_Utf8(null);
		$s->addChar($character);
		return $s->toString();
...
function com_wiris_quizzes_impl_HTMLTools_10(&$»this, &$appendpos, &$character, &$contentpos, &$end, &$firstchar, &$formula, &$initag, &$length, &$nextpos, &$nexttag, &$nexttaglength, &$nexttagname, &$parentpos, &$parenttag, &$parenttagname, &$pos, &$speciallength, &$specialtag, &$start, &$text, &$toappend) {
	{
		$s = new haxe_Utf8(null);
		$s->addChar($character);
		return $s->toString();
...
function com_wiris_quizzes_impl_HTMLTools_11(&$»this, &$c, &$end, &$html, &$name, &$pos) {
	{
		$s = new haxe_Utf8(null);
		$s->addChar($c);
		return $s->toString();
