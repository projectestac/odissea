...
class script_health extends agora_script_base{
    ...
    public function params(){
        $params = array();
        $params['get'] = optional_param('get', false, PARAM_TEXT);
        return $params;
    }
    ...
    protected function _execute($params = array(), $execute = true) {
        ...
        if (!is_dir($CFG->dataroot) || !is_writable($CFG->dataroot)) {
             echo $OUTPUT->notification('Your <strong>config.php</strong> says that your "data root" directory is <strong>'.$CFG->dataroot.'</strong>. However, this directory either does not exist or cannot be written to by Moodle. This means that a variety of problems will be present, such as users not being able to log in and not being able to upload any files. It is imperative that you address this problem for Moodle to work correctly.');
        }
        ...
        if (!empty($agora)) {
            if ($params['get'] == 'agora') {
                print_collapsible_region_start('', 'agora',$OUTPUT->heading('Variable $agora'), '', true);
                print_object($agora);
                print_collapsible_region_end();
            }
        }
