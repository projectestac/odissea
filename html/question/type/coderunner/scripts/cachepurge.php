...
/**
 * Purge Coderunner grading cache entries by context.
 *
 * When called without parameters, this script lists all contexts accessible
 * to the current user that have grading cache entries, with buttons to purge
 * old or all entries. When called with contextid and usettl parameters it
 * performs the purge for that context.
 *
...
define('ALLBUTTONTEXT', get_string('purgeallcachekeysbutton', 'qtype_coderunner')); // For button to purge all entries.
...
$usettl = optional_param('usettl', -1, PARAM_INT);
...
if ($contextid > 0 && $usettl >= 0) {
    ...
    $title = get_string('cachepurgepagetitle', 'qtype_coderunner', $context->get_context_name());
    ...
    $purger = new cache_purger($usettlbool);
    echo $OUTPUT->header();
    echo $OUTPUT->heading($title, 4);
    $purger->purge_cache_for_context($context->id);
} else {
    // Index mode: list contexts with grading cache entries.
    cache_config_writer::update_definitions();
    $PAGE->set_url('/question/type/coderunner/scripts/cachepurge.php');
    $PAGE->set_context($context);
                ...
                if (has_capability('moodle/question:editall', $coursecontext)) {
                    ...
                    if (count($allbanks) > 0) {
                        ...
                        foreach ($allbanks as $qbank) {
                            ...
                            $name = $ctx->get_context_name(true, true);
