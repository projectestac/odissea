...
class csv_import_reader {
    ...
    var $_fp;
//XTEC *********** AFEGIT -> Allow to set the faild path
...
//********** FI
     ...
     */
//XTEC *********** MODIFICAT -> Add $filepath to he constructor
//2011.01.26 @mmartinez
    function csv_import_reader($iid, $type, $filepath) {
//********** ORIGINAL
...
//XTEC ************ MODIFICAT -> To take filepath and delimiter defined in the class
//2011.01.26 @mmartinez
        $filename = $this->_filepath.$this->delimiter.$this->_iid;
//*********** ORIGINAL
...
//XTEC ************ MODIFICAT -> To take filepath and delimiter defined in the class
// 2011.01.26 @mmartinez
        $filename = $this->_filepath.$this->delimiter.$this->_iid; echo "$filename";
//*********** ORIGINAL
        //$filename = $CFG->dataroot.'/temp/csvimport/'.$this->_type.'/'.$USER->id.'/'.$this->_iid;
//*********** FI
...
//XTEC ************ MODIFICAT -> To take filepath and delimiter defined in the class
//2011.01.26 @mmartinez
            @unlink($this->_filepath.$this->delimiter.$this->_iid);
//************ ORIGINAL
...
//************ FI
     ...
     * @return int iid
     */
    function get_new_iid($filepath, $filename, $type) {
        global $USER, $CFG;

//XTEC ************* AFEGIT -> to set variables when they aren't defined
//2011.01.26 @mmartinez
    	if ((empty($this->_filepath) || $this->_filepath != $filepath) && !empty($filepath)){
...
//XTEC *********** MODIFICAT -> To take the filepath defined in the class
//2011.01.26 @mmartinez
        if (!$filename = csv_import_reader::make_upload_directory($this->_filepath)) {
//************ ORIGINAL
...
//XTEC ************ MODIFICAT -> To take filepath and delimiter defined in the class
//2011.01.26 @mmartinez
        while (file_exists($this->_filepath.$this->delimiter.$filename)) {
//*********** ORIGINAL
...
//XTEC ************ AFEGIT -> Personalize make_upload_directory
//2011.01.26 @mmartinez
    /**
	 * Create a directory.
	 *
	...
	function make_upload_directory($directory) {
	    ...
	    $dirarray = explode($this->delimiter, $directory);
	    $currdir = '';
	    
	    foreach ($dirarray as $dir) {
	        $currdir = ($currdir != '') ? $currdir .$this->delimiter. $dir : $dir;
