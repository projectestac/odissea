...
/**
 * This class manage a ftp connection
 * 
...
class ftp4p{
     ...
     */
    function __construct($host = '', $user = '', $pass = '', $permanent = true, $debug = false){
...
//********** FI
        ...
        if (!$flist = ftp_nlist($this->connection, $path)){
            if ($this->islogger){
    ...
    function get_file ($remotefile = '', $localfile = '', $returnhandler = true){
        ...
        //store debug path
        if ($this->debug){
            $this->logger->add('ftp4.class.php: Getting file...', 'DEBUG');
        }
...
////////////////////////////////////////////////////////
	...
	private function get_logger($debug = false){
		...
		$this->logger = new log4p(false, 'log/ftp4p.log');
		if ($debug){		    	
		    $this->debug = $debug;
		}
		return true;
