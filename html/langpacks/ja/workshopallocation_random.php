...
/**
 * Strings for component 'workshopallocation_random', language 'ja', version '4.5'.
 *
...
$string['allocationdeallocategraded'] = 'すでに評価された提出の割り当てはできません: 評価者 <strong>{$a->reviewername}</strong> / 提出作成者 <strong>{$a->authorname}</strong>';
...
$string['assessmentdeleteddetail'] = '提出が再度割り当てられました: <strong>{$a->reviewername}</strong> は <strong>{$a->authorname}</strong> の評価者ではなくなりました。';
$string['assesswosubmission'] = '参加者は何も提出せずに評価できる';
$string['confignumofreviews'] = 'ランダムに割り当てられるデフォルトの提出数です。';
$string['excludesamegroup'] = '同一グループ内の他のメンバによるレビューを抑制する';
$string['noallocationtoadd'] = '追加できる割り当てはありません。';
$string['nogroupusers'] = '<p>警告: ワークショップに「可視グループ」または「分離グループ」が設定されていない場合、ユーザが自分に割り当てられた相互評価を実施するにはこのツールにより少なくとも1つのグループに属する必要があります。グループなしのユーザには新しい自己評価を割り当てることができます。そうではない場合、既存の評価は削除されます。</p>
<p>現在、これらのユーザはグループに属していません: {$a}</p>';
$string['numofdeallocatedassessment'] = '{$a} 件の評価を再割り当てする';
...
$string['privacy:metadata'] = 'ランダム割り当てプラグインはいかなる個人データも保存しません。誰が誰を評価したかという実際の個人データはワークショップモジュール自体に保存されます。データは評価詳細をエクスポートすることを目的として整形されます。';
