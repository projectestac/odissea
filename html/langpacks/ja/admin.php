...
$string['allowframembedding_help'] = 'この設定を有効にした場合、「LTIツールとして公開する」登録プラグイン使用時に推奨されるようにこのサイトをリモートシステム内のフレームに埋め込むことができます。そうではない場合、セキュリティ上の理由からフレーム埋め込みを無効のままにすることをお勧めします。モバイルアプリの場合、この設定が無視されること、フレーム埋め込みが常に有効にされることにも留意してください。';
...
$string['configallcountrycodes'] = '例えばユーザプロファイルのように、これは多くの場所で選択できる国リストです。空白にした場合 (デフォルト)、標準英語言語パックのcountries.php内リストが使用されます。このリストはISO 3166-1から作成されています。そうではない場合、例えば「JA,GB,FR,ES」 のように、カンマ区切りのコードのリストを記述することができます。あなたが新しい非標準コードをここに入力した場合、「en」内のcontries.phpおよびあなたの言語パックに追加する必要があります。';
...
$string['configallowguestmymoodle'] = 'この設定を有効にした場合、ゲストがダッシュボードにアクセスできるようになります。そうではない場合、ゲストはサイトホームにリダイレクトされます。';
...
$string['configautolangusercreation'] = 'この設定を有効にした場合、初回ログイン (例 LDAPまたはOAuth 2認証を使用) でユーザアカウントが自動作成される時にユーザのブラウザ言語が優先言語として設定されます。そうではない場合、サイトのデフォルト言語がユーザの優先言語として設定されます。';
...
$string['configcookiesecure'] = 'サーバがhttps接続のみ受け付ける場合、セキュアクッキーの送信を有効にすることをお勧めします。この設定を有効にした場合、ウェブサーバがhttp://を受け付けないこと、またはhttps://アドレスにパーマネントにリダイレクトされていることをご確認ください。また、理想的にはHSTSヘッダを送信してください。<em>wwwroot</em>アドレスがhttps://で始まらない場合、この設定は自動的に無効にされます。';
...
$string['configextendedusernamechars'] = 'この設定を有効にした場合、大文字以外、ユーザ名にどのような文字でも使用できます。そうではない場合、英数字小文字、アンダースコア (_)、ハイフン (-)、ピリオド (.) またはアットマーク (@) が許可されます。';
$string['configextramemorylimit'] = '検索、バックアップ/リストアまたはcronのようなスクリプトではさらなるメモリを必要とします。規模の大きなサイトでは設定値を上げてください。';
$string['configfilterall'] = 'ヘッダ、タイトル、ナビゲーションバー等を含むすべての文字をフィルタします。これは多言語フィルタを使用している時に最も有用です。そうではない場合、あなたのサイトに単に負荷がかかるだけになります。';
...
$string['configlangstringcache'] = 'すべての言語ファイルをデータディレクトリ内のコンパイル済みファイル内にキャッシュします。あなたがMoodleを翻訳またはMoodleソースコード内のストリングを変更している場合、無効にしてください。そうではない場合、パフォーマンスを向上させるため有効のままにしてください。';
...
$string['configproxyhost'] = '<b>サーバ</b>のインターネットアクセスにプロキシ (例 ファイアウォール) が必要な場合、プロキシのホスト名を入力してください。そうではない場合、空白にしてください。';
$string['configproxylogunsafe_help'] = 'ここではプロキシを通らない内部コールの記録を試みます。';
$string['configproxypassword'] = 'プロキシ経由でインターネットにアクセスする必要がある場合、パスワードを入力してください。そうではない場合、空白にしてください (PHP cURL拡張モジュールが必要です)。';
...
$string['configproxyuser'] = 'プロキシ経由でインターネットにアクセスする必要がある場合、ユーザ名を入力してください。そうではない場合、空白にしてください (PHP cURL拡張モジュールが必要です)。';
...
$string['coursecontactduplicates_desc'] = 'この設定を有効にした場合、選択された複数のコース連絡先ロールのユーザがロールと共にコース説明に表示されます。そうではない場合、 (どれがサイト管理の「ロールを定義する」で上位に表示されていたとしても) 1つのロールのみが表示されます。';
...
$string['datarootsecuritywarning'] = 'あなたのサイト設定は安全ではない可能性があります。datarootディレクトリ ({$a}) がウェブから直接アクセスできないことをご確認ください。';
...
$string['debugdisplay'] = 'デバッグメッセージを表示する';
...
$string['defaultformatnotset'] = 'デフォルトコースフォーマットの決定時にエラーが発生しました。サイト設定をご確認ください。';
$string['defaulthomepage'] = 'ユーザ用開始ページ';
...
$string['devicedetectregex_desc'] = '<p>デフォルトではMoodleはデフォルト (デスクトップPC、ラップトップ等)、モバイル (電話およびスモールハンドヘルドデバイス)、タブレット (iPad、Androidタブレット) およびレガシ (Internet Explorer 6ユーザ) のデバイスタイプを検出することができます。テーマセレクタはこれらすべてに異なるテーマを適用するために使用することができます。この設定では特別なデバイスタイプを検出するよう正規表現を使用できます (これはデフォルトタイプに優先します)。</p>
...
$string['editordictionary'] = 'エディタ辞書';
...
$string['enabledashboard_help'] = 'ダッシュボードにはデフォルトでタイムライン、カレンダー、最近アクセスしたアイテムが表示されます。あなたはすべての人に異なるデフォルトのダッシュボードを設定できます。また、ユーザが自分のダッシュボードをカスタマイズできるようにできます。 無効の場合、あなたは「ユーザ用開始ページ」をダッシュボード以外の値に設定する必要があります。';
...
$string['installsessionerror'] = 'PHPセッションを初期化できません、あなたのブラウザがクッキーを受け付けるかどうかご確認ください。';
...
サーバ「 {$a->sitename} 」内のあなたのアカウント (ユーザ名: {$a->username}  )  が無効な複数回のログインによりロックアウトされました。

すぐにアカウントをロック解除するには次のアドレスにアクセスしてください:

<a href="{$a->link}">アカウントをロック解除する</a>
...
$string['marketplaceavailablenotice'] = '<a href="{$a}" target="_blank" rel="noopener noreferrer">Moodle Marketplace <i class="fa fa-external-link" role="img" aria-label="Opens in new window" title="Opens in new window"></i></a>でプラグインを閲覧およびダウンロードしてください。';
...
$string['moodlenetremovalwarning'] = 'MoodleNetサービスは2026年4月20日に終了します。あなたのサイト上でMoodleNetを引き続き利用したい場合、<a href="https://github.com/moodlehq/moodle-tool_moodlenet" target="_blank">Moodle HQ GitHub repository <i class="fa fa-external-link" role="img" aria-label="Opens in new window" title="Opens in new window"></i></a>からMoodleNetプラグインをインストールしてセルフホスト型MoodleNetインスタンスに接続してください。その後、MoodleNetプロファイルIDフィールドは削除されます。あなたが他の目的で使用している場合、そのデータを移行してください。';
$string['moodlepartners'] = 'Moodleパートナ';
$string['moodlepartners_help'] = '<p>Moodle HQにより認定されたMoodleパートナはあなたの組織のオンライン学習環境に質の高いMoodleサービスを提供します。</p>
<a href="https://moodle.com/about-partners/">私たちのパートナに関して</a><br />
...
$string['navigationupgrade'] = 'このアップグレードでは次のブロックに替えて、新しい2つのナビゲーションブロックが導入されます: 管理、コース、活動および参加者。これらのブロックに特別なパーミッションを設定している場合、すべてがあなたの希望どおりに動作するかご確認ください。';
...
$string['navsortmycourseshiddenlast_help'] = 'この設定を有効にした場合、すべての非表示コースは表示コースの後に一覧表示されます (非表示コースを表示できるユーザに対して)。そうではない場合、すべてのコースは可視性にかかわらず、「マイコースを並べ替える」設定に基づき一覧表示されます。';
...
$string['oracledatabaseinuse'] = 'Oracle DBサポートは削除されます。Moodle 4.5はOracle DBサポートの最終バージョンとなります。詳細はmoodle.orgのアナウンスメント「<a href="https://moodle.org/mod/forum/discuss.php?d=456122">LMSにおけるオラクルデータベースサポートの廃止 (Oracle database support in LMS deprecation)</a>」をご覧ください。';
$string['order1'] = '1番目';
$string['order2'] = '2番目';
$string['order3'] = '3番目';
$string['order4'] = '4番目';
...
$string['pathtogs_help'] = 'ほとんどのLinuxインストールの場合、ここは「/usr/bin/gs」のままにしてください。Windowsでは「c:\\gs\\bin\\gswin32c.exe」のようになります (パスの間にスペースがないことをご確認ください - 必要であれば、ファイル「gswin32c.exe」および「gsdll32.dll」をパス内にスペースを入れずに新しいフォルダにコピーしてください)。';
$string['pathtopdftoppm'] = 'pdftoppmのパス';
$string['pathtopdftoppm_help'] = '<a href="https://poppler.freedesktop.org/">Poppler</a>はPDF レンダリングライブラリでPDFファイルをPNGに変換するツールpdftoppmが含まれます。一般的にGhostscriptを使用した場合よりパフォーマンスが良く、特に大きなファイルの場合に良くなります。利用可能な場合、Ghostscriptに優先してpdftoppmが使用されます。ほとんどのLinuxインストールではパスを「/usr/bin/pdftoppm」のままにできます。そうではない場合、あなたのLinuxディストリビューションに応じてpoppler-utilsまたはpopplerパッケージをインストールする必要があります。WindowsではCygwinインストールで提供されます。';
...
$string['pleaserefreshregistrationnewdata'] = '登録情報が更新されました。「登録を更新する」ボタンを使用して内容をご確認ください。';
$string['pleaserefreshregistrationunknown'] = 'あなたのサイトは登録されましたが、日付が不明です。あなたの登録を「登録を更新する」ボタンを使用して更新するかあなたの登録を自動更新するよう「サイト登録」スケジュールタスクが有効にされているかご確認ください。';
$string['plugin'] = 'プラグイン';
...
$string['rememberuserlicensepref_help'] = 'この設定を有効にした場合、ユーザが最後に選択したライセンスがファイルピッカでのファイルアップロード時に事前選択されます。そうではない場合、デフォルトサイトライセンスが事前選択されます。';
...
$string['searchallavailablecoursesdesc'] = '登録済みコースのみを検索するよう設定した場合、コース情報 (名称および概要) およびコースコンテンツのみユーザが登録しているコースで検索されます。そうではない場合、例えばゲストアクセスが有効にされているコースのようにユーザがアクセスできるコースすべてのコース情報およびコースコンテンツが検索されます。';
...
$string['testoutgoingmailconf_errorcommunications'] = 'あなたのサイトはあなたのメールサーバに接続できませんでした。あなたの送信メール設定をご確認ください。';
...
$string['upgradesettingsintro'] = '以下に表示されている設定はあなたの直近のMoodleアップグレードで追加されました。必要に応じてデフォルトから変更した後、このページ下部にある「変更を保存する」ボタンをクリックしてください。';
