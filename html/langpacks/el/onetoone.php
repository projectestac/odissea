<?php
// This file is part of Moodle - https://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <https://www.gnu.org/licenses/>.

/**
 * Strings for component 'onetoone', language 'el', version '4.4'.
 *
 * @package     onetoone
 * @category    string
 * @copyright   1999 Martin Dougiamas and contributors
 * @license     https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

$string['addremoveattendees'] = 'Προσθήκη/αφαίρεση παρόντων';
$string['addsession'] = 'Προσθήκη μιας νέας συνόδου';
$string['addstudent'] = 'Προσθήκη μαθητή';
$string['alllocations'] = 'Όλες οι τοποθεσίες';
$string['allsessionsin'] = 'Όλες οι σύνοδοι σε {$a}';
$string['approvalreqd'] = 'Απαιτείται έγκριση';
$string['approvalreqd_help'] = 'Όταν είναι επιλεγμένο το «Απαιτείται έγκριση», ένας εκπαιδευόμενος θα χρειάζεται έγκριση από τον διευθυντή του για να του επιτραπεί να παρακολουθήσει μια ένας-προς-έναν σύνοδο.';
$string['approve'] = 'Έγκριση';
$string['assessmentyour'] = 'Η αξιολόγησή σας';
$string['attendance'] = 'Παρουσίες';
$string['attendanceinstructions'] = 'Επιλέξτε χρήστες παρόντες στην σύνοδο:';
$string['attendedsession'] = 'Παρόντας στη σύνοδο';
$string['attendees'] = 'Παρόντες';
$string['attendeestablesummary'] = 'Άνθρωποι που σχεδιάζουν να είναι ή ήταν παρόντες σε αυτή τη σύνοδο.';
$string['cannotapproveatcapacity'] = 'Δεν μπορείτε να εγκρίνετε κι άλλους παρόντες καθώς η σύνοδος αυτή είναι πλήρης.';
$string['changemanageremailinstruction'] = 'Παρακαλούμε, δώστε παρακάτω την διεύθυνση ηλε.ταχυδρομείου του διευθυντή σας.';
$string['clearall'] = 'Καθαρισμός όλων';
$string['closed'] = 'Κλειστό';
$string['conditions'] = 'Συνθήκες';
$string['confirm'] = 'Επιβεβαίωση';
$string['confirmation'] = 'Επιβεβαίωση';
$string['confirmationmessage'] = 'Μήνυμα επιβεβαίωσης';
$string['confirmationmessage_help'] = 'Αυτό το μήνυμα αποστέλλεται όποτε κάποιος χρήστης κάνει κράτηση για μια σύνοδο.';
$string['confirmationsent'] = 'Θα πρέπει να λάβετε αμέσως ένα μήνυμα ηλε.ταχυδρομείου επιβεβαίωσης.';
$string['confirmationsentmgr'] = 'Εσείς και ο διευθυντής σας θα πρέπει να λάβετε αμέσως ένα μήνυμα ηλε.ταχυδρομείου επιβεβαίωσης.';
$string['confirmmanageremailaddress'] = 'Επιβεβαίωση διεύθυνσης ηλε.ταχυδρομείου διευθυντή';
$string['confirmmanageremailaddressquestion'] = 'Είναι ακόμα η διεύθυνση <b>{$a}</b> η διεύθυνση ηλε.ταχυδρομείου του διευθυντή σας;';
$string['confirmmanageremailinstruction1'] = 'Προηγουμένως δώσατε την ακόλουθη ως διεύθυνση ηλε.ταχυδρομείου του διευθυντή σας:';
$string['confirmmanageremailinstruction2'] = 'Είναι αυτή ακόμη η διεύθυνση ηλε.ταχυδρομείου του διευθυντή σας;';
$string['continuetologin'] = 'Κάντε κλικ στο πλήκτρο Συνέχεια, για να συνδεθείτε';
$string['copy'] = 'Αντιγραφή';
$string['copyingsession'] = 'Αντιγραφή ως μια νέα σύνοδος στο {$a}';
$string['copysession'] = 'Αντιγραφή συνόδου';
$string['cost'] = 'Κόστος';
$string['costheading'] = 'Κόστος συνόδου';
$string['currentstatus'] = 'Τρέχουσα κατάσταση';
$string['customfieldsheading'] = 'Προσαρμοσμένα πεδία συνόδου';
$string['date'] = 'Ημερομηνία';
$string['dateadd'] = 'Προσθήκη μιας νέας ημερομηνίας';
$string['dateremove'] = 'Διαγραφή αυτής της ημερομηνίας';
$string['datesignedup'] = 'Ημερομηνία εγγραφής';
$string['datetext'] = 'Είστε εγγεγραμμένος για την ημερομηνία';
$string['datetimeknownhinttext'] = '';
$string['decidelater'] = 'Απόφαση αργότερα';
$string['decline'] = 'Άρνηση';
$string['delete'] = 'Διαγραφή';
$string['deleteall'] = 'Διαγραφή όλων';
$string['deletesession'] = 'Διαγραφή συνόδου';
$string['deletesessionconfirm'] = 'Σίγουρα θέλετε να διαγράψετε αυτήν την σύνοδο και όλες τις εγγραφές για αυτή τη σύνοδο;';
$string['deletingsession'] = 'Διαγραφή συνόδου σε {$a}...';
$string['description'] = 'Κείμενο εισαγωγής';
$string['details'] = 'Λεπτομέρειες';
$string['discountcode'] = 'Κωδικός έκπτωσης';
$string['discountcost'] = 'Κόστος έκπτωσης';
$string['discountcosthinttext'] = '';
$string['due'] = 'οφειλόμενα';
$string['duration'] = 'Διάρκεια';
$string['early'] = '{$a} νωρίς';
$string['edit'] = 'Επεξεργασία';
$string['editingsession'] = 'Διαγραφή της συνόδου του {$a}';
$string['editsession'] = 'Επεξεργασία συνόδου';
$string['email:instrmngr'] = 'Ειδοποίηση για τον διευθυντή';
$string['email:message'] = 'Μήνυμα';
$string['email:subject'] = 'Θέμα';
$string['emailmanager'] = 'Αποστολή ειδοποίησης διευθυντή';
$string['emailmanagercancellation'] = '# Αποστολή ειδοποίησης στον διευθυντή';
$string['emailmanagerconfirmation'] = '# Αποστολή ειδοποίησης στον διευθυντή';
$string['emailmanagerconfirmation_help'] = 'Όταν επιλεγεί «Αποστολή ειδοποίησης στον διευθυντή», ένα μήνυμα επιβεβαίωσης θα σταλεί στον διευθυντή του εκπαιδευόμενου, λίγες μέρες πριν την ημερομηνία έναρξης της συνόδου ένας-προς-έναν.';
$string['emailmanagerreminder'] = '# Αποστολή ειδοποίησης στον διευθυντή';
$string['emailmanagerreminder_help'] = 'Όταν επιλεγεί «Αποστολή ειδοποίησης στον διευθυντή», ένα μήνυμα υπενθύμισης θα σταλεί στον διευθυντή του εκπαιδευόμενου, λίγες μέρες πριν την ημερομηνία έναρξης της συνόδου ένας-προς-έναν.';
$string['emptylocation'] = 'Η τοποθεσία ήταν άδεια';
$string['enrolled'] = 'εγγράφηκε';
$string['error:addalreadysignedupattendee'] = 'Ο/Η {$a} έχει ήδη εγγραφεί για αυτήν την ένας-προς-έναν δραστηριότητα.';
$string['error:addattendee'] = 'Αδυναμία προσθήκης {$a} στην σύνοδο.';
$string['error:cancelbooking'] = 'Παρουσιάστηκε πρόβλημα με την ακύρωσή της κράτησής σας';
$string['error:canttakeattendanceforunstartedsession'] = 'Δεν μπορείτε να καταγράψετε παρουσίες για μια σύνοδο που δεν έχει ακόμη αρχίσει.';
$string['error:couldnotaddfield'] = 'Αδυναμία προσθήκης προσαρμοσμένου πεδίου συνόδου.';
$string['error:couldnotaddnotice'] = 'Αδυναμία προσθήκης ειδοποίησης ιστοτόπου.';
$string['error:couldnotaddsession'] = 'Αδυναμία προσθήκης συνόδου';
$string['error:couldnotaddtrainer'] = 'Αδυναμία προσθήκης νέου εκπαιδευτή συνόδου ένας-προς-έναν';
$string['error:couldnotcopysession'] = 'Αδυναμία αντιγραφής συνόδου';
$string['error:couldnotdeletefield'] = 'Αδυναμία διαγραφής προσαρμοσμένου πεδίου συνόδου';
$string['error:couldnotdeletenotice'] = 'Αδυναμία διαγραφής ειδοποίησης ιστοτόπου.';
$string['error:couldnotdeletesession'] = 'Αδυναμία διαγραφής συνόδου';
$string['error:couldnotdeletetrainer'] = 'Αδυναμία διαγραφής νέου εκπαιδευτή συνόδου ένας-προς-έναν';
$string['error:couldnotfindsession'] = 'Αδυναμία εύρεσης της συνόδου που πρόσφατα προστέθηκε';
$string['error:couldnotsavecustomfield'] = 'Αδυναμία αποθήκευσης προσαρμοσμένου πεδίου';
$string['error:couldnotupdatecalendar'] = 'Αδυναμία ενημέρωσης γεγονότος συνόδου στο ημερολόγιο.';
$string['error:couldnotupdatefield'] = 'Αδυναμία ενημέρωσης προσαρμοσμένου πεδίου συνόδου.';
$string['error:couldnotupdatemanageremail'] = 'Αδυναμία ενημέρωσης διεύθυνσης ηλε.ταχυδρομείου του διευθυντή.';
$string['error:couldnotupdatenotice'] = 'Αδυναμία ενημέρωσης ειδοποίησης ιστοτόπου.';
$string['error:couldnotupdatesession'] = 'Αδυναμία ενημέρωσης συνόδου';
$string['error:coursemisconfigured'] = 'Το μάθημα έχει μη έγκυρες ρυθμίσεις';
$string['error:fieldidincorrect'] = 'Το αναγνωριστικό πεδίου είναι λανθασμένο: {$a}';
$string['error:incorrectcoursemodule'] = 'Το άρθρωμα μαθήματος είναι λανθασμένο';
$string['error:incorrectcoursemoduleid'] = 'Το αναγνωριστικό αρθρώματος μαθήματος ήταν λανθασμένο';
$string['error:invaliduserid'] = 'Μη έγκυρο αναγνωριστικό χρήστη';
$string['error:nomanagersemailset'] = 'Δεν έχει οριστεί διεύθυνση ηλε.ταχυδρομείου διαχειριστή';
$string['error:noticeidincorrect'] = 'Το αναγνωριστικό ειδοποίησης είναι λανθασμένο: {$a}';
$string['error:problemsigningup'] = 'Παρουσιάστηκε κάποιο πρόβλημα κατά την εγγραφή σας.';
$string['error:removeattendee'] = 'Αδυναμία απομάκρυνσης {$a} από την σύνοδο.';
$string['error:sessionstartafterend'] = 'Η ημερομηνία/ώρα έναρξης συνόδου είναι μετά την λήξη.';
$string['error:shortnametaken'] = 'Υπάρχει ήδη ένα προσαρμοσμένο πεδίο με αυτό το σύντομο όνομα.';
$string['error:signedupinothersession'] = 'Έχετε ήδη εγγραφεί σε μια άλλη σύνοδο για αυτή τη δραστηριότητα. Μπορείτε να εγγραφείτε μόνο για μία σύνοδο ανά δραστηριότητα ένας-προς-έναν.';
$string['excelformat'] = 'Excel';
$string['export'] = 'Εξαγωγή';
$string['exportattendance'] = 'Εξαγωγή παρουσιών';
$string['exporttofile'] = 'Εξαγωγή σε αρχείο';
$string['feedback'] = 'Ανατροφοδότηση';
$string['feedbackupdated'] = 'Οι ανατροφοδοτήσεις ενημερώθηκαν για \\{$a} άτομα';
$string['field:multiselect'] = 'Πολλαπλή επιλογή';
$string['field:select'] = 'Μενού επιλογών';
$string['field:text'] = 'Κείμενο';
$string['floor'] = 'Πάτωμα';
$string['format'] = 'Μορφότυπος';
$string['goback'] = 'Επιστροφή';
$string['import'] = 'Εισαγωγή';
$string['info'] = 'Πληροφορίες';
$string['location'] = 'Τοποθεσία';
$string['lookfor'] = 'Αναζήτηση';
$string['manageremailinstructionconfirm'] = 'Παρακαλούμε, επιβεβαιώστε ότι αυτή είναι η διεύθυνση ηλε.ταχυδρομείου του διευθυντή σας:';
$string['managername'] = 'Όνομα διευθυντή';
$string['managerupdated'] = 'Η διεύθυνση ηλε.ταχυδρομείου του διευθυντή σας ενημερώθηκε.';
$string['maximumpoints'] = 'Μέγιστος αριθμός πόντων';
$string['maximumsize'] = 'Μέγιστος αριθμός παρόντων';
$string['none'] = 'κανένα';
$string['normalcosthinttext'] = '';
$string['note'] = 'Σημείωση';
$string['onehour'] = '1 ώρα';
$string['oneminute'] = '1 λεπτό';
$string['onetoone'] = 'Ένας-με-έναν';
$string['onetoone:addattendees'] = 'Προσθήκη παρόντων σε μια ένας-με-έναν σύνοδο.';
$string['onetoone:editsessions'] = 'Προσθήκη, επεξεργασία, αντιγραφή και διαγραφή συνόδων ένας-με-έναν';
$string['onetoone:overbook'] = 'Εγγραφή για πλήρεις συνόδους.';
$string['onetoone:removeattendees'] = 'Αφαίρεση παρόντων από μια ένας-προς-έναν σύνοδο';
$string['onetoone:signup'] = 'Εγγραφή για μια σύνοδο';
$string['onetoone:takeattendance'] = 'Δήλωση παρουσιών';
$string['onetoone:view'] = 'Εμφάνιση δραστηριοτήτων και συνόδων ένας-προς-έναν';
$string['onetoone:viewattendees'] = 'Εμφάνιση λίστας παρουσιών και παρόντων';
$string['onetoone:viewcancellations'] = 'Εμφάνιση ακυρώσεων';
$string['onetoone:viewemptyactivities'] = 'Εμφάνιση κενών δραστηριοτήτων ένας-προς-έναν';
$string['onetoonebooking'] = 'Κράτηση ένας-προς-έναν';
$string['onetoonename'] = 'Όνομα ένας-προς-έναν';
$string['onetoonesession'] = 'Σύνοδος ένας-προς-έναν';
$string['options'] = 'Επιλογές';
$string['or'] = 'ή';
$string['order'] = 'Παραγγελία';
$string['place'] = 'Αίθουσα';
$string['placeholder:alldates'] = '[ΌλεςΟιΗμερομηνίες]';
$string['placeholder:attendeeslink'] = '[σύνδεσμοςπαρουσιών]';
$string['placeholder:cost'] = '[κόστος]';
$string['placeholder:details'] = '[λεπτομέρειες]';
$string['placeholder:duration'] = '[διάρκεια]';
$string['placeholder:finishtime'] = '[χρόνοςτέλους]';
$string['placeholder:firstname'] = '[μικρόόνομα]';
$string['placeholder:lastname'] = '[επώνυμο]';
$string['placeholder:sessiondate'] = '[ημερομηνίασυνόδου]';
$string['placeholder:starttime'] = '[χρόνοςέναρξης]';
$string['pluginname'] = 'Ένας-με-έναν';
$string['points'] = 'Πόντοι';
$string['pointsplural'] = 'Πόντοι';
$string['potentialattendees'] = 'Πιθανοί παρόντες';
$string['previoussessions'] = 'Προηγούμενες σύνοδοι';
$string['printversionid'] = 'Έκδοση εκτύπωσης: χωρίς όνομα';
$string['printversionname'] = 'Έκδοση εκτύπωσης: με όνομα';
$string['registeredon'] = 'Καταχωρήθηκε στις';
$string['registrations'] = 'Καταχωρήσεις';
$string['reminder'] = 'Υπενθύμιση';
$string['reminderinstrmngr'] = '# Ειδοποίηση για διευθυντή';
$string['reminderinstrmngr_help'] = 'Όταν επιλεγεί **Αποστολή ειδοποίησης στον διευθυντή**, το κείμενο στο πεδίο **Ειδοποίηση για διευθυντή** αποστέλλεται στον διευθυντή ενός εκπαιδευόμενου, πληροφορώντας τον ότι ο εκπαιδευόμενος έχει εγγραφεί για μια σύνοδο ένας-προς-έναν.';
$string['remindermessage'] = 'Μήνυμα υπενθύμισης';
$string['remindermessage_help'] = 'Αυτό το μήνυμα αποστέλλεται μερικές ημέρες πριν μια σύνοδο ένας-προς-έναν.';
$string['reminderperiod'] = 'Ημέρες πριν για αποστολή μηνύματος';
$string['reminderperiod_help'] = 'Το μήνυμα υπενθύμισης θα αποστέλλεται τόσες ημέρες πριν την έναρξη της συνόδου.';
$string['requestmessage'] = 'Μήνυμα αιτήματος';
$string['requeststablesummary'] = 'Άτομα που αιτούνται να παρακολουθήσουν αυτήν την σύνοδο.';
$string['room'] = 'Αίθουσα';
$string['saveallfeedback'] = 'Αποθήκευση όλων των αποκρίσεων';
$string['saveattendance'] = 'Αποθήκευση παρουσιών';
$string['scheduledsession'] = 'Χρονοπρογραμματισμένη σύνοδος';
$string['scheduledsessions'] = 'Χρονοπρογραμματισμένες σύνοδοι';
$string['seatsavailable'] = 'Διαθέσιμες θέσεις';
$string['seeattendees'] = 'Δείτε παρόντες';
$string['sentremindermanager'] = 'Αποστολή μηνύματος (ηλε.ταχυδρομείου) υπενθύμισης στον διευθυντή του χρήστη';
$string['sentreminderuser'] = 'Αποστολή μηνύματος (ηλε.ταχυδρομείου) υπενθύμισης στον χρήστη';
$string['sessiondate'] = 'Ημερομηνία συνόδου είναι η ημερομηνία κατά την οποία ξεκινά η σύνοδος.';
$string['sessiondatetime'] = 'Ημερομηνία/ώρα συνόδου';
$string['sessiondatetimeknown'] = 'Ημερομηνία/ώρα συνόδου γνωστή';
$string['sessionfinishtime'] = 'Ώρα λήξης συνόδου';
$string['sessioninprogress'] = 'σύνοδος σε εξέλιξη';
$string['sessionisfull'] = 'Αυτή η σύνοδος είναι τώρα πλήρης. Θα πρέπει να διαλέξετε άλλη ώρα ή να μιλήσετε με τον εκπαιδευτή.';
$string['sessionover'] = 'λήξη συνόδου';
$string['sessionrequiresmanagerapproval'] = 'Για να γίνει κράτηση σε αυτή τη σύνοδο, απαιτείται έγκριση διευθυντή.';
$string['sessionroles'] = 'Ρόλοι συνόδου';
$string['sessions'] = 'Σύνοδοι';
$string['sessionsdetailstablesummary'] = 'Πλήρης περιγραφή της τρέχουσας συνόδου.';
$string['sessionsoncoursepage'] = 'Σύνοδοι που εμφανίζονται στην σελίδα μαθήματος';
$string['sessionsoncoursepage_help'] = 'Αυτός είναι ο αριθμός των συνόδων για κάθε δραστηριότητα ένας-με-έναν που θα εμφανιστεί στην κύρια σελίδα μαθήματος.';
$string['sessionstartdate'] = 'Ημερομηνία έναρξης συνόδου';
$string['sessionstarttime'] = 'Ώρα έναρξης συνόδου';
$string['sessionvenue'] = 'Τόπος συνόδου';
$string['setting:addchangemanageremail'] = 'Ερώτηση προς χρήστες για την διεύθυνση ηλε.ταχυδρομείου του διευθυντή τους.';
$string['setting:addchangemanageremail_caption'] = 'Διεύθυνση ηλε.ταχυδρομείου διευθυντή:';
$string['setting:addchangemanageremaildefault'] = 'Ερώτηση προς χρήστες για την διεύθυνση ηλε.ταχυδρομείου του διευθυντή τους.';
$string['setting:defaultcancellationinstrmngr'] = 'Προεπιλεγμένο μήνυμα ακύρωσης που αποστέλλεται στους διευθυντές.';
$string['setting:defaultremindermessage_caption'] = 'Μήνυμα υπενθύμισης';
$string['setting:manageraddressformatdefault'] = '';
$string['status'] = 'Κατάσταση';
$string['submissions'] = 'Υποβολές';
$string['submit'] = 'Υποβολή';
$string['summary'] = 'Περίληψη';
$string['takeattendance'] = 'Δήλωση παρουσιών';
$string['time'] = 'Ώρα';
$string['unapprovedrequests'] = 'Μη εγκεκριμένα ερωτήματα';
$string['unknowndate'] = '(άγνωστη ημερομηνία)';
$string['unknowntime'] = '(άγνωστη ώρα)';
$string['upcomingsessions'] = 'Επερχόμενες σύνοδοι';
$string['updaterequests'] = 'Αιτήματα ενημέρωσης';
$string['xhours'] = '{$a} ώρες';
$string['xminutes'] = '{$a} λεπτά';
