...
/**
 * Strings for component 'admin', language 'ru', version '3.11'.
 *
 * @package     admin
 * @category    string
...
$string['configautolangusercreation'] = 'Если этот параметр включен, то при автоматическом создании учетной записи пользователя при первом входе в систему (например, с использованием аутентификации LDAP или OAuth 2), язык браузера устанавливается в качестве предпочтительного языка пользователя. При отключенном параметре предпочтительным языком пользователя устанавливается язык по умолчанию для сайта.';
...
$string['configcustomusermenuitems'] = 'Вы можете настроить содержимое меню пользователя (за исключением ссылки «Выход», которая добавляется автоматически). Каждая строка разделяется символами | и состоит из: 1) строки в форме «идентификаторстроки, названиекомпонента» или обычного текста, 2) адреса URL и 3) иконки в виде картинки или значка пикселя (в папке pix со структурой [имя_подпапки]/[имя_иконки], например, i/publish) или в виде URL. Также возможно создание разделительной линии путем добавления в нужном месте строки из одного или нескольких символов #.';
...
$string['debugsqltrace'] = 'Показать начало вызовов SQL';
$string['debugsqltrace1'] = 'Показать только одну строку вызова';
...
$string['messagingdeleteallnotificationsdelay'] = 'Удалять все сообщения через';
$string['messagingdeletereadnotificationsdelay'] = 'Удалять прочитанные сообщения через';
...
$string['xmlrpcmaharaenabled'] = 'Обнаружено, что на вашем сайте включено электронное портфолио Mahara. Эта функция зависит от расширения PHP XML-RPC, которое PHP больше не поддерживает.';
$string['xmlrpcmnetenabled'] = 'Обнаружено, что на вашем сайте включено сетевое взаимодействие Moodle. Эта функция зависит от расширения PHP XML-RPC, которое PHP больше не поддерживает.';
