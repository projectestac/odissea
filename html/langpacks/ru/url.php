...
/**
 * Strings for component 'url', language 'ru', version '4.5'.
 *
 * @package     url
 * @category    string
...
$string['allowvariables'] = 'Разрешить переменные в URL';
$string['allowvariables_desc'] = 'Разрешить добавление переменных в URL-адреса. Переменные позволяют передавать внутреннюю информацию, например имя пользователя, как часть URL-адреса. Учитывайте потенциальных рисках конфиденциальности при использовании этой функции.';
...
$string['configrolesinparams'] = 'Должны ли пользовательские имена ролей (из настроек курса) быть доступны в качестве переменных для параметров URL?';
...
* Внедрить - гиперссылка отображается на странице вместе с описанием
...
* В фрейме - гиперссылка отображается в фрейме под описанием
* В новой вкладке - гиперссылка открывается в новой вкладке браузера с меню и адресной строкой';
...
$string['name_help'] = 'Это будет текст ссылки для URL.

Введите информативный текст, кратко описывающий назначение URL.

Избегайте использования слова «ссылка». Это поможет пользователям программ чтения с экрана, так как программы чтения с экрана объявляют ссылки (например, «Moodle.org, ссылка»), поэтому нет необходимости включать слово «ссылка» в поле названия.';
...
$string['rolesinparams'] = 'Имена ролей в качестве переменных URL';
