...
الكتابة في المخدم.';
$string['uninstalldeleteconfirm'] = 'تم حذف كل البيانات المرتبطة بالملحق <em>{$a->name}</em> من قاعدة البيانات. لمنع هذا الملحق من إعادة تنصيب نفسه، يجب إزالة مجلده <em>{$a->rootdir}</em> من المخدم. هل تريد إزالة مجلد هذا الملحق الآن؟';
...
$string['updatepluginconfirminfo'] = 'أنت على وشك تنصيب إصدار جديد من الملحق <strong>{$a->name}</strong>. سيتم تنزيل حزمة zip ذات الإصدار {$a->version} من الملحق من <a href="{$a->url}">{$a->url}</a> وسيتم استخراجها إلى تنصيب لتتمكن من ترقية التنصيب الخاص بك.';
...
$string['validationmsg_rootdir_help'] = 'يمثل اسم المجلد الجذر في حزمة ZIP اسم الملحق المراد تنصيبه. إذا كان الاسم غير صحيح، فقد ترغب في إعادة تسمية المجلد الجذر في ملف ZIP قبل تنصيب الملحق.';
