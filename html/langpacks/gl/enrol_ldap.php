...
/**
 * Strings for component 'enrol_ldap', language 'gl', branch 'MOODLE_38_STABLE'
 *
 * @package   enrol_ldap
...
$string['idnumber_attribute'] = 'Se os membros do grupo teñen nomes distintos, especifique o mesmo atributo que usou para a asignación do «Número ID» do usuario na configuración de autenticación LDAP.';
...
$string['pluginname_desc'] = '<p>Pode utilizar un servidor LDAP para controlar as súas inscricións. É asumido que a súa árbore LDAP contén grupos que asignan os cursos, e que cada un deses grupos/cursos terán entradas de membros para asignar alumnos.</p><p>É asumido que os cursos son definidos como grupos en LDAP, e que cada grupo ten múltiples campos de membros (<em>member</em> ou <em>memberUid</em>) que conteñen unha única identificación de usuario.</p><p>Para utilizar a inscrición LDAP, os seus usuarios <strong>deben</strong> ter un campo de número id válido. Os grupos LDAP deben ter este número id nos campos de membro para que un usuario poida inscribirse no curso. Normalmente, isto funciona ben se xa está a utilizar a Autenticación LDAP.</p><p>As inscricións serán actualizadas cando os usuarios inicien a sesión. Tamén pode executar un script para manter as inscricións sincronizadas. Vexa en <em>enrol/ldap/cli/sync.php</p><p>Este engadido tamén pode ser definido para crear automaticamente novos cursos ao aparecer grupos novos en LDAP.</p>';
$string['pluginnotenabled'] = 'Engadido non activado!';
$string['privacy:metadata'] = 'O engadido de LDAP non almacena ningún dato persoal.';
$string['role_mapping'] = '<p>Por cada rol que queira asignar desde  LDAP, debe especificar a lista de contextos onde se atopan os grupos de roles do curso. Separe os contextos diferentes con «;».</p><p>Tamén necesita especificar o atributo que os seus servidores LDAP utilizan para conter os membros dun grupo. Adoita ser «member» ou «memberUid»</p>';
...
$string['template'] = 'Opcional: os cursos creados automaticamente poden copiar as súas configuracións dun modelo de curso.';
