 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class subsectioncmitem implements named_templatable, renderable {
    ...
    public function export_for_template(\renderer_base $output): stdClass {
        ...
        $itemdata = $item->export_for_template($output);
        $labelname = $itemdata->activityname;
        $labelnum = '&nbsp';
        if (preg_match('/^(\w{1,5})\.(.*)$/', trim($itemdata->activityname), $match)) {
            $labelnum = $match[1];
            $labelname = trim($match[2]);
        }
        $itemdata->labelnum = $labelnum;
        $itemdata->name = format_string($labelname);
    ...
    public function get_template_name(\renderer_base $renderer): string {
        return 'format_vv/local/content/section/subsectioncmitem';
    }
}
